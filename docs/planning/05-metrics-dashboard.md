...
| Active Alerts | 현재 firing 알림 집계 | 룰 기반 장애 감지 확인 | 알림 유형별 SRE/Infra/Developer | Prometheus | `sum(ALERTS{alertstate="firing"})` | `>= 1` |
| API 5xx Ratio (5m) | 서버 오류 비율 | 요청 실패 증가 확인 | 상승 시 SRE 이동 | Prometheus | `mwa:http_5xx_ratio:5m` | `> 0.03` |
| API p95 Latency (5m) | 상위 지연 시간 | 체감 느림 확인 | 상승 시 SRE, 필요 시 Infra | Prometheus | `mwa:http_latency_p95_seconds:5m` | `> 1.5s` |
...
| Container Memory Usage | 컨테이너 메모리 | OOM/GC 압박 확인 | 한계 근접 시 restart/OOM 확인 | Prometheus | `container_memory_working_set_bytes` | 한계 근접 |
...
| DB Lock Wait / Deadlocks | DB 락 압력 | DB가 살아도 요청 정체 가능 | 증가 시 SQL/트랜잭션 추적 | Prometheus | `pg_stat_database_deadlocks` | deadlock 증가 |
...
| 패널 | 역할 | 왜 보는가 | 다음 판단 | 데이터 소스 | 주요 쿼리/메트릭 | 경고 기준 |
|---|---|---|---|---|---|---|
| Top 5 Slow API | 느린 API 식별 | 지연 집중 endpoint 압축 | trace drill-down | Prometheus | `mwa_http_request_duration_seconds_*` | 상위 지연 증가 |
| Failed Request Table | 실패 요청 목록 | 실패 요청 구체 확인 | trace_id 선택 후 추적 | Loki | `{compose_service="mwa-backend"} | json | status_code >= 500` | 실패 이벤트 증가 |
| Recent Failed Traces | 실패 trace 진입점 | 오류 계층 빠른 파악 | span 분석 | Tempo(링크) | Explore 링크 | TODO(자동 목록) |
| Trace-selected Logs | trace 연동 로그 | trace/log 문맥 결합 | 예외/검증/외부오류 확인 | Loki | `trace_id =~ "$trace_id"` | 오류 이벤트 증가 |
...
| Top Error Signatures | 에러 패턴 상위 | 우선 대응 대상 결정 | 대표 시그니처 대응 | Loki | `count_over_time(...error_code...)` | 특정 코드 burst |
...
| Search Result Exists Ratio | 결과 존재 비율 | 기술 성공/비즈니스 성공 분리 | 급락 시 인덱싱/품질 점검 | Prometheus | `mwa:search_result_exists_ratio:5m` | 급락 추세 |
| Cart Add Success | 장바구니 성공률 | 전환 중간 단계 확인 | 저하 시 API/재고/세션 점검 | Prometheus | `mwa:cart_add_success_ratio:5m` | `< 99.5%` |
| Order Success | 주문 성공률 | 핵심 매출 이벤트 확인 | 저하 시 Developer | Prometheus | `mwa:order_create_success_ratio:5m` | `< 99.0%` |
| Payment Completion | 결제 완료율 | 실질 매출 성공 판단 | 저하 시 의존성/장애 영향 분석 | Prometheus | `mwa:payment_completion_ratio:5m` | `< 95%` |
| Orders Created (1h) | 실주문량 | 비율 밖 거래량 감소 확인 | baseline 대비 비교 | Prometheus | `increase(mwa_order_create_total{result="success"}[1h])` | baseline 하회 |
| Conversion Delta vs Baseline | 전환 변화량 | 정상 변동/이상 감소 구분 | 유의미 감소 시 장애 영향 판단 | Prometheus | `mwa:conversion_delta_vs_baseline:5m` | `< -2%p` |
| Estimated Revenue Loss | 추정 손실 금액 | 기술 지표의 비즈니스 번역 | 손실 증가 시 우선순위 상향 | Prometheus | `mwa:estimated_revenue_loss_per_hour:5m` | TODO(미계측) |

## 16. 미계측 TODO 패널

- `Worker Queue Lag`: 큐 exporter 또는 app gauge 연동 필요.
- `Thread/Pool Saturation`: 런타임 thread/connection pool metric 연동 필요.
- `Recent Failed Traces` 자동 리스트: Tempo 검색 API 기반 패널 필요.
