...
export class CartService {
	...
	private async findActiveCartId(entityManager: EntityManager, customerId: string): Promise<string | null> {
	...
	private async ensureActiveCartId(entityManager: EntityManager, customerId: string): Promise<string> {
	...
	private async loadVariant(entityManager: EntityManager, variantId: string): Promise<VariantRow> {
		const rows = await entityManager.getConnection().execute<VariantRow>(
			VARIANT_LOOKUP_SQL,
			[variantId],
			'all',
		);
	...
	private async loadCartView(entityManager: EntityManager, cartId: string): Promise<CartView> {
		const rows = await entityManager.getConnection().execute<CartRow>(CART_VIEW_SQL, [cartId], 'all');
	...
	public async getCart(customerId: string): Promise<CartView> {
		return this.withCustomerLock(customerId, async (entityManager) => {
			const cartId = await this.ensureActiveCartId(entityManager, customerId);
			return this.loadCartView(entityManager, cartId);
		});
	}
	...
	public async addItem(customerId: string, variantId: string, quantity: number): Promise<CartView> {
		return this.withCustomerLock(customerId, async (entityManager) => {
			const cartId = await this.ensureActiveCartId(entityManager, customerId);
			const variant = await this.loadVariant(entityManager, variantId);
			...
			if (existing !== undefined) {
				...
				if (variant.available_quantity < nextQuantity) {
					throw createUnavailableError('Cart item is unavailable', {
						variant_id: variantId,
						available_quantity: variant.available_quantity,
					});
				...
				[nextQuantity, existing.cart_item_id],
				'run',
				);
				await entityManager.getConnection().execute(
	...
	public async updateItem(customerId: string, cartItemId: string, quantity: number): Promise<CartView> {
		return this.withCustomerLock(customerId, async (entityManager) => {
			const activeCartId = await this.findActiveCartId(entityManager, customerId);
			if (activeCartId === null) {
				throw new NotFoundError('Cart item not found');
			}

			const cart = await this.loadCartView(entityManager, activeCartId);
			...
			return this.loadCartView(entityManager, cart.cart_id);
		});
	}
