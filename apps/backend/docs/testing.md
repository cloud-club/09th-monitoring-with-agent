...
- `*.spec.ts`: unit tests (fast, pure domain/service behavior)
- `*.it.spec.ts`: integration tests (module wiring/public interface with local dependencies)
- `*.e2e.spec.ts`: API e2e tests (Playwright request flows)
